...
from models.mmh_vig.torch_han import AV_HAN
from models.mmh_vig.torch_vertex import Grapher
from models.mmh_vig.torch_nn import act_layer
from models.mmh_vig.torch_edge import dense_knn_matrix
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        max_dilation = (256 + 48) // max(num_knn)

        self.image_stem = Stem(out_dim=channels, act=act)
        self.audio_stem = Stem(out_dim=channels, act=act)
        self.img_pos_embed = nn.Parameter(torch.zeros(1, channels, 14, 14))
        self.audio_pos_embed = nn.Parameter(torch.zeros(1, channels, 6, 8))
        ...
        self.han = AV_HAN(input_size=channels, hidden_size=channels, output_size=channels)
        self.han_norm = nn.BatchNorm2d(channels)
        ...
        self.prediction = nn.Linear(channels, opt.n_classes)
        self.model_init()
    ...
    def forward(
        ...
        if image_embeds is not None and audio_embeds is not None:
            x_img = image_embeds[:, 1:, :].transpose(1, 2)
            x_audio = audio_embeds[:, 1:, :].transpose(1, 2)
        else:
            x_img = self.image_stem(image_inputs) + self.img_pos_embed
            x_audio = self.audio_stem(audio_inputs) + self.audio_pos_embed

        x_img = x_img.reshape(x_img.shape[0], x_img.shape[1], -1, 1)
        x_audio = x_audio.reshape(x_audio.shape[0], x_audio.shape[1], -1, 1)

        x = torch.cat([x_img, x_audio], dim=2)
        B, C, H, W = x.shape
        ...
        # HAN
        _tmp = x
        edge_index = dense_knn_matrix(x, k=32)
        x = x.squeeze(-1).permute(0, 2, 1)
        x = self.han(x, edge_index)
...
@register_model
def vig_ti16_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0, num_knn=16, **kwargs):
            self.k = num_knn  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            self.act = 'gelu'  # activation layer {relu, prelu, leakyrelu, gelu, hswish}
            self.norm = 'batch'  # batch or instance normalization {batch, instance}
...
@register_model
def vig_b512_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=512, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            self.act = 'gelu'  # activation layer {relu, prelu, leakyrelu, gelu, hswish}
            self.norm = 'batch'  # batch or instance normalization {batch, instance}
...
@register_model
def vig_h_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1024, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            self.act = 'gelu'  # activation layer {relu, prelu, leakyrelu, gelu, hswish}
            self.norm = 'batch'  # batch or instance normalization {batch, instance}

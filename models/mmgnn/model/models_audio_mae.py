...
from cross_attn_block import CrossBlock
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def patchify(self, imgs):
        ...
        if self.audio_exp:
            if self.use_custom_patch:  # overlapped patch
                ...
                x = imgs.unfold(2, self.patch_size, self.stride).unfold(3, self.patch_size,
                                                                        self.stride)  # n,1,H,W -> n,1,h,w,p,p
    ...
    def forward_encoder(self, x, mask_ratio, mask_2d=False):
        ...
        if mask_2d:
            x, mask, ids_restore = self.random_masking_2d(x, mask_t_prob=self.mask_t_prob,
                                                          mask_f_prob=self.mask_f_prob)
        else:
    ...
    def forward_decoder(self, x, k, v, ids_restore):
        # embed tokens
        ...
        if self.decoder_mode != 0:
            raise NotImplementedError("Only support decoder_mode=0")
        else:
            # apply Transformer blocks
        ...
        if self.decoder_mode != 0:
            ...
            pred = pred[:, 1:, :]
        return pred, x, None  # emb, emb_pixel
    ...
    def forward(self, imgs, k=None, v=None, mask_ratio=0.8):
